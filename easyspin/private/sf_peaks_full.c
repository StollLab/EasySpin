/*
sf_peaks_full(IncSchemeID,buffer,dt,E,G,D,M1,M1t)
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  double *rT2l, *iT2l, *rT2r, *iT2r, *rT3l, *iT3l, *rT3r, *iT3r, *rT4l, *iT4l, *rT4r, *iT4r;
  double *E;
  double rAmp, iAmp, rAmp0, iAmp0, rAmp1, iAmp1, rAmp2, iAmp2, rAmp3, iAmp3, rAmp4, iAmp4, rAmp5, iAmp5, rAmp6, iAmp6, rAmp7, iAmp7;
  double *rSpec, *iSpec;
  ...
  switch (IncSchemeID) {
    ...
    case  4: nFreeEvolutions = 4; nDimensions = 1; break;
    case  5: nFreeEvolutions = 4; nDimensions = 1; break;
    case  6: nFreeEvolutions = 4; nDimensions = 1; break;
    ...
    case 18: nFreeEvolutions = 3; nDimensions = 2; break;
    case 19: nFreeEvolutions = 5; nDimensions = 2; break;
    case 20: nFreeEvolutions = 5; nDimensions = 2; break;
    case 21: nFreeEvolutions = 5; nDimensions = 2; break;
  ...
  /* E ... energies */
  E = mxGetPr(prhs[a]);
  if (mxIsComplex(prhs[a])) mexErrMsgTxt("E must be real!");
  ...
  if (nFreeEvolutions==1) {
    if (nrhs!=6) mexErrMsgTxt("Wrong number of input arguments.");
  }
  else  if (nFreeEvolutions==2) {
    ...
    a++;
    if (mxGetNumberOfElements(prhs[a])!=nStates*nStates)
      mexErrMsgTxt("M4l has wrong size!");
    rT4l = mxGetPr(prhs[a]);
    iT4l = mxIsComplex(prhs[a]) ? mxGetPi(prhs[a]) : mxGetPr(imagZeros);
    ...
    a++;
    if (mxGetNumberOfElements(prhs[a])!=nStates*nStates)
      mexErrMsgTxt("M4r has wrong size!");
    rT4r = mxGetPr(prhs[a]);
    iT4r = mxIsComplex(prhs[a]) ? mxGetPi(prhs[a]) : mxGetPr(imagZeros);
  }
  else
  ...
  switch (IncSchemeID) {
    ...
    case 11: /* [1 2], e.g. HYSCORE*/
      ...
      for (j=0;j<nStates;j++) {
        for (i=0;i<nStates;i++) {
           ...
           * evolution goes with (-omega_ij)t, but Fourier theory
           * uses (+omega_ij)t */
          id1 = fmod(-(E[i]-E[j])*dt1,1.0)*nPoints1;
          id2 = fmod(-(E[i]-E[j])*dt2,1.0)*nPoints2;
                ...
                for (o=0;o<nStates;o++) {
                  ...
                  for (i=0;i<nStates;i++) {
                    ...
                    for (p=0;p<nStates;p++) {
                      ...
                      /* nu = (E1l[l]-E1r[m])+(E2l[k]-E2r[n])-(E3l[k]-E3r[n])-(E4l[j]-E4r[o]) */
                      nu = (E[l]-E[m]) + (E[k]-E[n]) - (E[j]-E[o]) - (E[i]-E[p]);
                      id1 = fmod(-nu*dt1,1.0)*nPoints1;
                      if (id1>=0) id1 = floor(id1);
                      else { id1 = ceil(id1); if (id1<0) id1 += nPoints1; }
                      idx = id1;             
                      
                      if ((idx<0)||(idx>nPoints1-1))
    ...
    case 18: /* [1 -1 2], e.g. DEER with echo detection */
    ...
    /*-------------------------------------------------------------------*/
    case 19: /* [1 1 -1 -1 2] e.g. SIFTER with echo detection */
      
      /* Amp = D[qh]*T4l[hi]*T3l[ij]*T2l[jk]*T1l[kl]*G[lm]*T1r[mn]*T2r[no]*T3r[op]*T4r[pq] */
      for (l=0;l<nStates;l++) {
        for (m=0;m<nStates;m++) {
          lm = l + m*nStates;
          for (k=0;k<nStates;k++) {
                ...
                for (o=0;o<nStates;o++) {
                  ...
                  for (i=0;i<nStates;i++) {
                    ...
                    for (p=0;p<nStates;p++) {
                      ...
                      /* nu1 = (E1l[l]-E1r[m])+(E2l[k]-E2r[n])-(E3l[k]-E3r[n])-(E4l[j]-E4r[o]) */
                      nu1_ = (E[l]-E[m]) + (E[k]-E[n]) - (E[j]-E[o]) - (E[i]-E[p]);
                      id1 = fmod(-nu1_*dt1,1.0)*nPoints1;
                      if (id1>=0) id1 = floor(id1);
                      else { id1 = ceil(id1); if (id1<0) id1 += nPoints1; }
                      ...
                      for (h=0;h<nStates;h++) {
                        hi = h + i*nStates;
                        /* Amp6 = T4l[hi]*Amp5 */
                        rAmp6 = rT4l[hi]*rAmp5 - iT4l[hi]*iAmp5;
                        iAmp6 = rT4l[hi]*iAmp5 + iT4l[hi]*rAmp5;
                        for (q=0;q<nStates;q++) {
                          pq = p + q*nStates;
                          qh = q + h*nStates;
                          /* Amp7 = Amp6*T4r[pq] */
                          rAmp7 = rAmp6*rT4r[pq] - iAmp6*iT4r[pq];
                          iAmp7 = rAmp6*iT4r[pq] + iAmp6*rT4r[pq];
                          rAmp = rD[qh]*rAmp7 - iD[qh]*iAmp7;
                          iAmp = rD[qh]*iAmp7 + iD[qh]*rAmp7;
    ...
    /*-------------------------------------------------------------------*/
    case 20: /* [1 -1 -1 1 2] e.g. 7p DEER with echo detection */
