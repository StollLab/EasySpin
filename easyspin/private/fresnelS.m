...
% X must be real or imaginary, but of any shape.
if any(imag(X) ~= 0 & real(X) ~=0 )
  error('FRESNELS:complexarguments','X should be real or imaginary.')
end

% Convert imaginary input to real-valued input
idxImag = real(X)==0 & imag(X)~=0;
X(idxImag)= imag(X(idxImag));
...
FSint(idxImag) = -1i*FSint(idxImag);
