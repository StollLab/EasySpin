...
s=[1/2 1/2];
Sx = sop(s,'xe'); Sy = sop(s,'ye'); Sz = sop(s,'ze');
Ix = sop(s,'ex'); Iy = sop(s,'ey'); Iz = sop(s,'ez');
v0 = 9.34567e3;
v2 = 15;
t = .01;
angle = pi;
...
tt = rand + [0 t];
tic; for k=1:10, [U2,UUU] = propint(F,G,tt,v0); end; toc
U2*Sz*U2'
tic; for k=1:10, U2 = propint(UUU,tt,v0); end; toc
...
n = 3:100;
for q=1:numel(n)
  U2{q} = propint(F,G,tt,v0,n(q));
end
for q=1:numel(n)
  err(q) = norm(abs(U2{q})-abs(U2{numel(n)}));
end
plot(n,err);
