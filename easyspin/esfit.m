...
nParams = size(data,1);
for p = 1:nParams
  oldvaluestring = striphtml(data{p,5});
...
% Remove displayed uncertainties
...
for p = 1:size(Data,1)
  Data{p,5} = '-'; 
  if ~esfitdata.fixedParams(p) && ~isempty(esfitdata.bestpstd)
...
if ~isempty(str)
  ...
  if k>0
    ...
    for p = 1:numel(values)
      data{p,6} = sprintf('%0.6g',values(p));
    end
    set(hTable,'Data',data);
...
lbColumn = 3; % lower-bound column
ubColumn = 4; % upper-bound column
...
ParTableColw = 85;
ParTablew = 8*ParTableColw+hElement+dh;
...
% Parameter table
%-------------------------------------------------------------------------------
columnname = {'','Name','lower','upper','current','best','std','ci95 lower','ci95 upper'};
columnformat = {'logical','char','char','char','char','char','char','char','char'};
colEditable = [true false true true false false false false false];
data = cell(numel(esfitdata.pinfo),9);
for p = 1:numel(esfitdata.pinfo)
  data{p,1} = true;
  data{p,2} = char(esfitdata.pinfo(p).Name);
  data{p,3} = sprintf('%0.6g',esfitdata.pvec_lb(p));
  data{p,4} = sprintf('%0.6g',esfitdata.pvec_ub(p));
  data{p,5} = '-';
  data{p,6} = '-';
  data{p,7} = '-';
  data{p,8} = '-';
...
uitable('Tag','ParameterTable',...
    ...
    'ColumnWidth',{hElement,ParTableColw,ParTableColw,ParTableColw,ParTableColw,ParTableColw,ParTableColw,ParTableColw,ParTableColw},...
    'RowName',[],...
    'Data',data);
x0shift = ParTablew-3*wButton1;
uicontrol('Style','text',...
    'Position',[ParTablex0 ParTabley0+ParTableh x0shift hElement],...
    'BackgroundColor',get(gcf,'Color'),...
    'FontWeight','bold','String','Parameters',...
    'HorizontalAl','left');
uicontrol('Style','pushbutton','Tag','selectInvButton',...
    'Position',[ParTablex0+x0shift ParTabley0+ParTableh wButton1 hElement],...
...
uicontrol('Style','pushbutton','Tag','selectAllButton',...
    'Position',[ParTablex0+x0shift+wButton1 ParTabley0+ParTableh wButton1 hElement],...
...
uicontrol('Style','pushbutton','Tag','selectNoneButton',...
    'Position',[ParTablex0+x0shift+2*wButton1 ParTabley0+ParTableh wButton1 hElement],...
...
uicontrol(hFig,'Style','text',...
    ...
    'BackgroundColor',get(gcf,'Color'),...
    'Position',[ParTablex0 ParTabley0+ParTableh+hElement wOptionsLabel hElement]);
uicontrol(hFig,'Style','text','Tag','statusText',...
    ...
    'BackgroundColor',get(gcf,'Color'),...
    'Position',[ParTablex0+ParTablew-wOptionsLabel ParTabley0+ParTableh+hElement wOptionsLabel hElement]);
uicontrol(hFig,'Style','text',...
    ...
    'HorizontalAlign','left',...
    'BackgroundColor',get(gcf,'Color'),...
    'Position',[ParTablex0+wOptionsLabel ParTabley0+ParTableh+hElement wOptionsLabel hElement]);
...
uicontrol(hFig,'Style','text',...
    ...
    'HorizontalAlign','left',...
    'BackgroundColor',get(gcf,'Color'),...
    'Position',[Optionsx0 Optionsy0+5*(hElement+dh)-dh+0.5*hElement wOptionsLabel hElement]);
