...
Exp.mwPhase = pi/2;
[nu,chi1_sim] = garlic(Sys,Exp);
chi1_sim = chi1_sim/max(abs(chi1_sim));

if opt.Display
  tiledlayout(2,1)
  nexttile
  plot(nu_GHz,chi1,nu_GHz,chi1_sim,'--',nu_GHz,(chi1_sim-chi1)*10);
  axis tight
  xlabel('frequency (GHz)')
  legend('explicit','garlic','diff*10')
  title('dispersion')

  nexttile
  plot(nu_GHz,chi2,nu_GHz,chi2_sim,'--',nu_GHz,(chi2_sim-chi2)*10);
  axis tight
  xlabel('frequency (GHz)')
  legend('explicit','garlic','diff*10')
  title('absorption')
