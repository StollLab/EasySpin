...
FrameTrajFile = load('MTSSL_polyAla_traj.mat');

tScale = 2.5;  % diffusion constant of TIP3P model water molecules in MD 
               % simulations is ~2.5x too high, so we scale the time axis

MD = FrameTrajFile.Traj;  % initialize the MD structure using loaded data

MD.tScale = 2.5;          % diffusion constant of TIP3P model water 
                          % molecules in MD  simulations is ~2.5x too high, 
                          % so we scale the time axis
MD.removeGlobal = 0;      % protein was fixed during MD simulation
